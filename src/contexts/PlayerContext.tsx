...
export const PlayerProvider: React.FC<Props> = ({ children }) => {
  ...
  const addPlayer = () => {
    setPlayers((prevPlayers) => {
      const maxId = prevPlayers.reduce((max, player) => Math.max(max, player.id), 0);
      return [...prevPlayers, { id: maxId + 1, gamesPlayed: 0, wins: 0, onBreak: false }];
    });
  };
  ...
  const updatePairHistoryByMatches = (matches: Match[]) => {
    ...
    matches.forEach((match) => {
      ...
      updatedPairHistory[p1.id][p2.id] = (updatedPairHistory[p1.id][p2.id] || 0) + 1;
      updatedPairHistory[p2.id][p1.id] = (updatedPairHistory[p2.id][p1.id] || 0) + 1;
      ...
      updatedPairHistory[p3.id][p4.id] = (updatedPairHistory[p3.id][p4.id] || 0) + 1;
      updatedPairHistory[p4.id][p3.id] = (updatedPairHistory[p4.id][p3.id] || 0) + 1;
    });
  ...
  const updateOpponentHistoryByMatches = (matches: Match[]) => {
    ...
    matches.forEach((match) => {
      ...
      updatedOpponentHistory[p1.id][p3.id] = (updatedOpponentHistory[p1.id][p3.id] || 0) + 1;
      updatedOpponentHistory[p1.id][p4.id] = (updatedOpponentHistory[p1.id][p4.id] || 0) + 1;
      updatedOpponentHistory[p2.id][p3.id] = (updatedOpponentHistory[p2.id][p3.id] || 0) + 1;
      updatedOpponentHistory[p2.id][p4.id] = (updatedOpponentHistory[p2.id][p4.id] || 0) + 1;

      updatedOpponentHistory[p3.id] = updatedOpponentHistory[p3.id] || {};
      updatedOpponentHistory[p4.id] = updatedOpponentHistory[p4.id] || {};
      updatedOpponentHistory[p3.id][p1.id] = (updatedOpponentHistory[p3.id][p1.id] || 0) + 1;
      updatedOpponentHistory[p3.id][p2.id] = (updatedOpponentHistory[p3.id][p2.id] || 0) + 1;
      updatedOpponentHistory[p4.id][p1.id] = (updatedOpponentHistory[p4.id][p1.id] || 0) + 1;
      updatedOpponentHistory[p4.id][p2.id] = (updatedOpponentHistory[p4.id][p2.id] || 0) + 1;
    });
